...
public class SendEvent
{
    public static class ConnectionStatus
    {
        ...
        public IotHubConnectionStatusChangeReason statusChangeReason;
        public Throwable throwable;

        public ConnectionStatus(IotHubConnectionStatus status, IotHubConnectionStatusChangeReason statusChangeReason, Throwable throwable)
        {
            this.status = status;
    ...
    public static ArrayList<ConnectionStatus> connectionStatuses = new ArrayList<>();
    ...
    protected static class IotHubConnectionStatusChangeCallbackLogger implements IotHubConnectionStatusChangeCallback
    {
        ...
        {
            ConnectionStatus connectionStatus = new ConnectionStatus(status, statusChangeReason, throwable);
    ...
    final static int numberOfMessagesToSend = 1000000; //1,000,000
    //final static int numberOfMessagesToSend = 100000; //100,000
    //final static int numberOfMessagesToSend = 10000; //10,000
    //final static int numberOfMessagesToSend = 1000; //1,000
    ...
    public static void main(String[] args) throws Exception
    {
        ...
        client.setOption("SetSASTokenExpiryTime", 3600l);
        ...
        {
            sendEventRunnables[sentMessageCount].run();
        }
        ...
        System.out.println("Shortest time between send and ack (seconds): " + calculateLowestSecondsBetweenSendAndAck());
        ...
        System.out.println("TOTAL LIST OF CONNECTION STATUS UPDATES");

        for (int i = 0; i < connectionStatuses.size(); i++)
        {
            connectionStatuses.get(i).printStatus();
        }
    }
    ...
    private static double calculateLowestSecondsBetweenSendAndAck()
    {
        double lowest = 99999999;
        for (int messageIndex = 0; messageIndex < numberOfMessagesToSend; messageIndex++)
        {
            double secondsTakenOnMessage = (stopTimes[messageIndex] - startTimes[messageIndex]) / 1000;
            if (secondsTakenOnMessage < lowest)
            {
                lowest = secondsTakenOnMessage;
            }
        }

        return lowest / numberOfMessagesToSend;
    }
}
